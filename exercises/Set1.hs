...
quadruple x = double x *2   -- aka x*2*2
...
distance :: Double -> Double -> Double -> Double -> Double -- takes in 4 Doubles, returns 1 Double
...
checkPassword password = if password == "swordfish" || password == "mellon" -- LOTR reference ^w^
                         then "You're in."
                         else "ACCESS DENIED!"
